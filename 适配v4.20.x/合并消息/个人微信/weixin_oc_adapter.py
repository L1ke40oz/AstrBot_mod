...
class WeixinOCAdapter(Platform):
    ...
    def __init__(
        ...
        # 用户发送分段消息的等待时间（秒）
        self.segment_wait_time: float = float(self.config.get("segment_input_wait_sec", 10))
    ...
    async def handle_msg(self, message: AstrBotMessage) -> None:
        """处理传入的消息，支持分段输入聚合。"""
        # 如果是空内容，直接提交
        if not message.message_str and not message.message:
    ...
    async def _schedule_processing(self, session_id: str) -> None:
        try:
    ...
    async def _process_buffered_messages(self, session_id: str) -> None:
        ...
        if len(message_list) > 1:
            ...
            final_message.timestamp = message_list[-1].timestamp

        logger.info(f"weixin_oc 聚合消息完毕 ({session_id}): {final_message.message_str}")
        await self._commit_message_event(final_message)

    async def _commit_message_event(self, message: AstrBotMessage) -> None:
        """统一提交事件的方法"""
        message_event = WeixinOCMessageEvent(
